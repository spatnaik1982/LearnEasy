...
export function jsonForConcept(concept: GeneratedConcept, activities: GeneratedActivity[]): string {
  const data: Record<string, unknown> = {
    conceptId: concept.conceptId,
    chapter: { code: concept.chapterCode, name: concept.chapterName },
    learningObjective: concept.learningObjective,
    coreIdea: concept.coreIdea,
    examples: concept.examples,
    misconceptions: concept.misconceptions,
...
export function writeConceptJSON(
  baseDir: string,
  levelCode: string,
  subject: string,
  concept: GeneratedConcept,
  ...
  const content = jsonForConcept(concept, activities);
  const tmpPath = join(tmpdir(), `curriculum-${concept.conceptId}-${Date.now()}.json`);
  writeFileSync(tmpPath, content, 'utf-8');
  renameSync(tmpPath, filePath);
  return filePath;
}

export function writeAllConceptsJSON(
  baseDir: string,
  levelCode: string,
  subject: string,
  entries: ConceptActivityPair[],
  ...
  for (const entry of entries) {
    try {
      const path = writeConceptJSON(baseDir, levelCode, subject, entry.concept, entry.activities, force);
      written.push(path);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      if (message.includes('already exists')) {
